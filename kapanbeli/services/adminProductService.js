    ...
    async createProduct(productData) {
        try {
            const {
                name,
                description,
                category,      // New: handle category name from frontend
                category_id,
                stock_quantity,
                min_stock_level,
                unit,
            ...
            // Resolve category_id: use category_id if provided, otherwise map from category name
            let categoryIdToUse = category_id;
            if (categoryIdToUse === undefined || categoryIdToUse === null) {
                if (typeof category === 'string') {
                    categoryIdToUse = categoryToIdMap[category] || 6;
                } else {
                    categoryIdToUse = 6; // Default to 'Lainnya'
                }
            }
            ...
            console.log('[adminProductService.createProduct] Executing query with values:', values);
