    ...
    async getAnalytics(req, res) {
        try {
            ...
            const [
                ...
                // Current period products - PRODUCT GROWTH
                db.execute(`
                    SELECT DATE_FORMAT(created_at, ?) as period, COUNT(*) as count
                    FROM products
                    WHERE DATE(created_at) >= DATE_SUB(CURDATE(), INTERVAL ? DAY)
            ...
            const productsMap = new Map(productsResult[0].map(r => [r.period, parseInt(r.count)]));
            ...
            const productsData = labels.map(label => {
                const key = this.getLabelKey(label, range, days);
                return productsMap.get(key) || 0;
            });
            ...
            const currProducts = productsResult[0].reduce((sum, r) => sum + parseInt(r.count), 0);
            const prevProducts = prevProductsResult[0][0].count;
            ...
            const response = {
                ...
                summary: {
                    ...
                    totalProducts: totalProductsResult[0][0].count,
                    totalLogins: currLogins,
                    dailyActiveUsers: dauResult[0][0].count,
                    newUsers: currUsers,
                    newProducts: currProducts,
                    growth: {
                        users: {
                            current: currUsers,
                            previous: prevUsers,
                        ...
                        products: {
                            current: currProducts,
                            previous: prevProducts,
                            percentage: this.calcGrowth(prevProducts, currProducts),
                            trend: currProducts >= prevProducts ? 'up' : 'down'
                        },
                        logins: {
                            current: currLogins,
                            previous: prevLogins,
