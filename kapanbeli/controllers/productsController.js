...
const productsController = {
    ...
    async update(req, res) {
        try {
            ...
            // Log to activity_logs
            const activityLogsService = require('../services/activityLogsService');
            await activityLogsService.log(userId, 'UPDATE', `Mengupdate produk "${updatedName}"`, req.ip || 'unknown', req.get('user-agent') || 'unknown');
    ...
    async delete(req, res) {
        try {
            ...
            // Note: We allow deletion of deactivated products because users should be able to remove 
            // unwanted items from their own list even if flagged by admin. 
            // We only block EDITING for deactivated products.
