...
const notesController = {
    ...
    async toggleStatus(req, res) {
        try {
            ...
            const activityLogsService = require('../services/activityLogsService');
            let statusAction = '';
            switch (updatedNote.status) {
                case 'Pending': statusAction = 'menunda'; break;
                case 'Selesai': statusAction = 'menyelesaikan'; break;
                case 'Belum Selesai': statusAction = 'membuka kembali'; break;
            }
            
            await activityLogsService.log(userId, 'UPDATE', `${statusAction} catatan: "${existingNote.title}"`, req.ip || 'unknown', req.get('user-agent') || 'unknown');

            res.status(200).json({
                success: true,
                message: `Status catatan diperbarui menjadi ${updatedNote.status}.`,
                note: updatedNote
            });
